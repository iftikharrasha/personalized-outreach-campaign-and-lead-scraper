...
export function EditCampaignModal({ open, campaign, onClose, onSaved, onArchived }: Props) {
  ...
  const submit = async () => {
    ...
    if (notifyEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(notifyEmail.trim())) errs.email = "That doesn't look like a valid email";
    setErrors(errs);
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.show({ type: "error", title: "Save failed", message: data.error });
        return;
      }
      toast.show({ type: "success", title: "Campaign updated", message: `Saved changes to "${name.trim()}".` });
      onSaved();
      onClose();
    } finally {
      setLoading(false);
  ...
  const handleArchive = async () => {
    ...
    try {
      ...
      toast.show({ type: "success", title: "Campaign archived", message: `${campaign.name} moved to Archived.` });
      onArchived();
      onClose();
    } finally {
      setLoading(false);
  ...
  return (
    <Modal open={open} onClose={onClose} width={580}>
      ...
      <div className="space-y-4">
        ...
        <Field label="Client email" hint="We don't scrape emails from Google Maps — add it here once the prospect shares it." error={errors.email}>
          <Input
            type="email"
            value={notifyEmail}
      ...
      <div className="mt-6 -mx-6 px-6 pt-5 border-t border-line dark:border-d-line flex items-center justify-between gap-2">
        <Button variant="ghost" onClick={handleArchive} leftIcon={<Archive size={14} />} className="text-mute hover:!text-negative">
          Archive campaign
        </Button>
        <div className="flex items-center gap-2">
          <Button variant="ghost" onClick={onClose}>Cancel</Button>
          <Button variant="primary" onClick={submit} loading={loading} disabled={!dirty} leftIcon={<Check size={14} />}>
            Save changes
          </Button>
        </div>
      </div>
